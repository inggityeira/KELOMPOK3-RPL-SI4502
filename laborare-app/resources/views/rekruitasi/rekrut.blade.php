...
@section('content')
        ...
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="/listsukarelawan"
                        style="color:white; font-size:20px; text-decoration: underline; letter-spacing:1px; font-size:18px;">Rekruitasi</a>
                </li>
                <li class="breadcrumb-item"><a href=""
                        style="color:white; font-size:20px; text-decoration: underline; letter-spacing:1px; font-size:18px;">Rekrut
                        Sukarelawan</a></li>
            </ol>
        </nav>
    </div>
    ...
    <div class="d-flex flex-column align-items-center  text-center mb-2">
        {{-- Foto Profil --}}
        <img src="{{ asset('pasfoto/' . $sukarelawan->pas_foto) }}" alt="Profile" class="img-fluid"
    ...
    <div class="container d-flex justify-content-center">
        <form style="width: 55%" method="post"
            action="{{ route('rekrut-sukarelawan', ['id' => $sukarelawan->id_sukarelawan]) }}">
            @csrf
            @method('put')
            <div class="mb-2">
                ...
                <div>
                    <select name="status_sukarelawan" class="card p-2" style="width: 100%; padding: 8px;">
                        <option value="Tidak Diterima"
                            {{ $sukarelawan->status_sukarelawan === 'Tidak Diterima' ? 'selected' : '' }}>Tidak Diterima
                        </option>
                        <option value="Diterima" {{ $sukarelawan->status_sukarelawan === 'Diterima' ? 'selected' : '' }}>
                            Diterima</option>
                        <option value="Selesai" {{ $sukarelawan->status_sukarelawan === 'Selesai' ? 'selected' : '' }}>
                            Selesai</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-group center-button mt-4" style="display: flex; justify-content: center;">
                <button type="submit" class="btn px-4"
                    style="background-color: white; color: black;"><strong>Selesai</strong></button>
            </div>
        </form>
