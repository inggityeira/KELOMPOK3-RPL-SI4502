...
@section('content')
    ...
    {{-- Notes: Tambahin href --}}
    <div class="col">
        <a href="{{ route('progresskegiatan') }}" id= "btn-kegiatan-yang-diikuti" class="btn btn-light fw-bolder"
            style="font-size:15px; margin-top:10px; margin-right:-100px;">Kegiatan yang diikuti</a>
    </div>
    ...
    <div class="d-flex justify-content-center" style="margin-top:15px; height:200px;">
        <form method="GET" action="{{ route('listkegiatan-Ind') }}">
            <div class="d-flex">
                <input name="search" class="form-control me-2" type="search" placeholder="Cari" aria-label="Search"
                    style="width: 330px; border-radius: 0px;">
                <button class="btn" type="submit"
                    style="background-color:#D9D9D9; margin-left:-8px; border-radius: 0px;"><img src="img/komponen/cari.png"
                        alt="Cari" width="30px"></button>
            </div>
            ...
            <div class="row centered" style="flex-wrap: wrap;">
                @foreach ($kegiatan->items() as $key => $item)
                    <div class="col-md-6 position-relative mb-4"
                        style="max-width: 360px; {{ $key == 0 && $kegiatan->count() == 1 ? 'margin-left: -320px;' : '' }}">
                        <a href="{{ route('detailkegiatan-Ind', $item->id_kegiatan) }}">
                            <img src="{{ asset('sampulkegiatan/' . $item->sampul_kegiatan) }}"
                                alt="{{ $item->nama_kegiatan }}" style=" width:340px; height:262px;">
                            <div class="fill-overlay">
                                <p class="card-text fw-bolder text-center text-white"
                                    style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width:100%; font-size:20px; weight: bold;">
